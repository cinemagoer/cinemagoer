...
__all__ = ['IMDb', 'IMDbError', 'Movie', 'available_access_systems']
__version__ = VERSION = '0.1dev20150925'
...
class IMDbBase:
    ...
    def get_imdbURL(self, mop):
        ...
        if imdbID is None:
            return None
        if isinstance(mop, Movie.Movie):
            url_firstPart = self.urls['movie_main']
        else:
            raise IMDbError('object ' + repr(mop) + ' is not a Movie')
