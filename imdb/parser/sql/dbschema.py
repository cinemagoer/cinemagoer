...
connection.cacheValues = False
sqlhub.processConnection = connection
createTables(uri)
#~ print CastInfo.sqlmeta.columns
#~ for x in KindType.select():
    #~ print x.id
#~ connection.debug = True
#~ print Title.q.kindID.fieldName
#~ for x in KindType.select():
    #~ print x.id, x.kind
#~ print KindType.byKind('movie')
#~ print KindType.byKind('movie')

#~ from copy import deepcopy
#~ Title.createTable()
#~ defaults = {}
#~ for x in Title.sqlmeta.columns.keys():
    #~ Key = Title.sqlmeta.columns[x].dbName
    #~ defaults[Key] = Title.sqlmeta.columns[x].default
#~ i = 0
#~ trans = connection.transaction()
#~ for t in ls:
    #~ val = deepcopy(defaults)
    #~ title = unicode(t.split('\t')[0], "latin_1")
    #~ val[Title.sqlmeta.columns['title'].dbName] = title
    #~ val[Title.sqlmeta.columns['kindID'].dbName] = None
    #~ val[Title.sqlmeta.idName] = i
    #~ i+=1
    #~ x=connection.sqlrepr(sqlbuilder.Insert(Title.sqlmeta.table, values=val))
    #~ connection.query(x)
#~ trans.commit()
