...
def test_search_movie_found_movie_should_have_imdb_index(ia):
    movies = ia.search_movie('blink', results=-1)
...
def test_search_movie_if_none_should_be_empty(ia):
    movies = ia.search_movie('%e3%82%a2', results=-1)
    assert movies == []
